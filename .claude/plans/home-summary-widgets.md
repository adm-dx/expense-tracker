...
1. **Данные предыдущего пользователя в той же вкладке (high).** Сторы категорий/транзакций/сводки сбрасывались только явным логаутом, а при 401 (`onAuthFailure`) и при входе другого пользователя — нет. Добавлен seam `shared/lib/store-reset.ts` (`registerStoreReset` / `resetRegisteredStores`), сторы регистрируют свой `reset`, а `entities/session` вызывает сброс в `setSession` и `clearSession`. Слои не нарушены: сторы не импортируют друг друга.
2. **Ответ «на лету» после сброса (medium).** `reset()` теперь инкрементит `latestRequestId` (в том числе в сторе категорий, где guard'а не было), поэтому запрос, отправленный до выхода, не заполнит очищенный стор.
3. **Регистрация не атомарна (medium).** Ошибка `CreateDefaultCategoriesCommand` больше не валит регистрацию: пользователь уже создан и email занят. Ошибка логируется через `Logger`, тесты — в `auth.service.spec.ts`.
4. **Последний день периода терялся (medium).** Фронтенд слал `dateTo` как полночь, API фильтрует `lte`. Добавлен `toIsoEndOfDay()` (`23:59:59.999Z`), сторы используют его. Проверено: транзакция от 30 сентября 18:45 попадает в сумму (337.50 → 347.49).
5. **Инвертированный диапазон в фильтре (low/medium).** `min`/`max` у `<input type="date">` не блокируют ввод; теперь вторая граница подтягивается за первой, и API не получает `dateFrom > dateTo`.
6. **Односторонний диапазон в summary (low).** Раньше недостающая граница подставлялась из текущего месяца (и `dateFrom` в будущем давал 400). Теперь `dateFrom` и `dateTo` обязательны вместе, как `month`+`year`.
7. **`page` без верхней границы (low).** `?page=99999999999` выходил за 32-битный `skip` Prisma и давал 500. Добавлен `@Max(MAX_TRANSACTION_PAGE)` → 400.
8. **Стор залипал в `loading` (low).** После отката страницы `fetch()` снова вызывает себя, а не полагается на эффект смонтированной таблицы.

Не приняты к исправлению: `DEFAULT_PERIOD` вычисляется при загрузке модуля (вкладка, открытая через смену месяца, покажет прежний период до перезагрузки).
